...
class TestLeadTimeCSVFile:
    @pytest.fixture(autouse=True)
    def around_each(self):
    ...
    def test_csv_created(self, capsys):
        ...
        self.filename, captured = get_csv_filename_and_captured_outerr(capsys)
        file_exists = os.path.exists(self.filename)
        assert "CSV metrics stored in lead_time_metrics_" in captured.out
        assert file_exists

    def test_column_names_in_csv_file(self, capsys):
        run_display_with_simple_builds()
        self.filename, captured = get_csv_filename_and_captured_outerr(capsys)
    ...
    def test_csv_github_commits(self, capsys):
        run_display_with_simple_builds()
        self.filename, captured = get_csv_filename_and_captured_outerr(capsys)
